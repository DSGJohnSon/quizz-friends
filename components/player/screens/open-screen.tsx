...
interface PlayerOpenScreenProps {
  session: SessionWithPlayers;
  player: Player;
  onLeave: () => void;
}

export function PlayerOpenScreen({
  session,
  player,
  onLeave,
}: PlayerOpenScreenProps) {
  return (
    <div className="h-screen flex items-center justify-center p-6 relative">
      ...
      <Card className="p-6 bg-white w-full text-center">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900 mb-4 uppercase flex flex-col items-center gap-2 mt-8">
            Vous êtes prêt !
          </h2>
          <p className="text-gray-600 mb-8 text-sm">
            Il ne reste plus qu&apos;à s&apos;échauffer pendant que les derniers
            candidats arrivent.
          </p>
        </div>
